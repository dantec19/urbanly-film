#!/usr/bin/env bash
# run-sim.sh plus the film's new-line scenario: the same bundle, region, years,
# seed and flags as the baseline, with the transit line and its transit-oriented
# upzone added (../line/transit-scenario.json, ../line/zoning-scenario.json).
#   run-sim-line.sh <region> <startYear> <endYear> <seed> <tag>
# Writes runs/<tag>/{stdout.log, mem.log, time.txt, run.ccsr, *.json} and a copy
# of both scenario files it ran with (runs/<tag>/scenario/).
set -uo pipefail
...
OUT="$SIM/runs/$TAG"
LINE="$REPO/.scratch/urbanly-film/data/line"
mkdir -p "$OUT/scenario"
cp "$LINE/transit-scenario.json" "$LINE/zoning-scenario.json" "$OUT/scenario/"
cd "$REPO"
...
/usr/bin/time -l node --max-semi-space-size=128 "$ENTRY" \
  ...
  --transit-scenario-file "$OUT/scenario/transit-scenario.json" \
  --zoning-scenario-file "$OUT/scenario/zoning-scenario.json" \
